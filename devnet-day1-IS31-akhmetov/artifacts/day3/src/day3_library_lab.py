...
def main() -> int:
    ...
    if not args.offline:
        ...
        for i in range(start_id, start_id + args.count):
            book = {"id": i, "title": f"{fake.catch_phrase()} [{th8}]", "author": st_name, "isbn": fake.isbn13()}
            scx, _ = add_book(s, api_key, book)
            if scx == 200:
                ok += 1
                if len(samples) < 5: samples.append(book)
            else: fail += 1
        ...
        add_report = {
            "generated_utc": now_utc(), "count_requested": args.count, "added_ok": ok, "added_fail": fail,
            "author_used": st_name, "tag": th8, "id_range": [start_id, start_id + args.count - 1],
            "sample_books": samples, "api_key_sha256": api_key_sha,
        }
        dump_json(add_report, ART / "add100_report.json")
        (ART / "add100_log_tail.txt").write_text(f"added_ok={ok}\nadded_fail={fail}\nid_range={start_id}..{start_id + args.count - 1}\n", encoding="utf-8")

    def sha_file_if_exists(p: Path) -> str:
        return sha256_text(p.read_text(encoding="utf-8")) if p.exists() else ""

    summary = {
        "schema_version": SUMMARY_SCHEMA_VERSION, "generated_utc": now_utc(),
        "student": {"token": st_token, "token_hash8": th8, "name": st_name, "group": st_group},
        "lab": {"apihost": APIHOST, "must_use": {"login_endpoint": f"{APIHOST}/api/v1/loginViaBasic", "books_endpoint": f"{APIHOST}/api/v1/books", "api_key_header": "X-API-KEY"}},
        "artifacts_sha256": {
            "books_before": sha_file_if_exists(ART / "books_before.json"), "books_sorted_isbn": sha_file_if_exists(ART / "books_sorted_isbn.json"),
            "mybook_post": sha_file_if_exists(ART / "mybook_post.json"), "books_by_me": sha_file_if_exists(ART / "books_by_me.json"),
            "add100_report": sha_file_if_exists(ART / "add100_report.json"), "postman_collection": sha_file_if_exists(ART / "postman_collection.json"),
            "postman_environment": sha_file_if_exists(ART / "postman_environment.json"), "curl_get_books": sha_file_if_exists(ART / "curl_get_books.txt"),
            "curl_get_books_isbn": sha_file_if_exists(ART / "curl_get_books_isbn.txt"), "curl_get_books_sorted": sha_file_if_exists(ART / "curl_get_books_sorted.txt"),
        },
        "validation": {"must_have_mybook_title_contains_token_hash8": True, "must_have_added_100": True},
    }
